...
# 台美醫院評論比較分析 - 打包腳本
# Taiwan-USA Hospital Review Comparison - Packaging Script

echo "======================================================================="
echo "📦 台美醫院評論比較分析 - 打包分享包"
echo "   Taiwan-USA Hospital Analysis - Creating Share Package"
echo "======================================================================="
echo ""

# 設定路徑
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PACKAGE_NAME="Taiwan_USA_Hospital_Analysis"
...
mkdir -p "$PACKAGE_DIR/results/usa_lda_k7"
mkdir -p "$PACKAGE_DIR/.streamlit"
mkdir -p "$PACKAGE_DIR/reports"

# 拷貝 Streamlit 應用程式
echo "📋 拷貝應用程式檔案..."
cp "$SCRIPT_DIR/code/streamlit_app/taiwan_usa_comparison.py" "$PACKAGE_DIR/code/streamlit_app/"
cp "$SCRIPT_DIR/code/streamlit_app/comparison_config.py" "$PACKAGE_DIR/code/streamlit_app/"

# 拷貝模型和資料
echo "📊 拷貝模型和資料檔案..."
cp "$SCRIPT_DIR/results/taiwan_lda_k7/lda_k7_lda_model.pkl" "$PACKAGE_DIR/results/taiwan_lda_k7/"
cp "$SCRIPT_DIR/results/usa_lda_k7/usa_gensim_lda_k6_model.pkl" "$PACKAGE_DIR/results/usa_lda_k7/"
cp "$SCRIPT_DIR/results/usa_lda_k7/usa_k6_topic_analysis_20251107_122236.csv" "$PACKAGE_DIR/results/usa_lda_k7/"
...
if [ -d "$SCRIPT_DIR/reports" ]; then
    cp "$SCRIPT_DIR/reports"/*.html "$PACKAGE_DIR/reports/" 2>/dev/null || true
fi
...
1. **互動式 Streamlit 應用程式** - 完整的比較分析系統
2. **HTML 靜態報告** - 可直接在瀏覽器開啟（位於 `reports/` 目錄）
3. **LDA 模型與資料** - 台灣 K=7 和美國 K=6 主題模型
...
如果不想安裝 Python，可以直接開啟靜態 HTML 報告：
...
echo "📦 打包內容:"
echo "   - Streamlit 應用程式"
echo "   - LDA 模型檔案 (Taiwan K=7, USA K=6)"
echo "   - HTML 靜態報告"
...
echo "   3. 等待瀏覽器自動開啟"
echo ""
echo "🌐 或直接開啟 HTML 報告（reports/ 目錄）"
echo "======================================================================="
EOF

chmod +x "$SCRIPT_DIR/pack_for_sharing.sh"

echo "✅ 打包腳本已建立！"
echo "📁 位置: $SCRIPT_DIR/pack_for_sharing.sh"
echo ""
echo "🚀 執行方式:"
echo "   chmod +x pack_for_sharing.sh"
echo "   ./pack_for_sharing.sh"
