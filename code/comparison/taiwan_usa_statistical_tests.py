...
"""
台美醫院評論跨國統計檢驗分析
Taiwan-USA Cross-National Statistical Tests
生成日期: 2025-11-07
"""
...
# ============================================================================
# Part 1: 載入台灣資料並生成主題分配
# ============================================================================
print("\n【Part 1】載入台灣 K=7 模型和資料...")

# 載入台灣原始資料
taiwan_data_path = '../../data/cleaned/taiwan/cleaned_data_no_dedup/final_cleaned_sample_no_dedup.csv'
df_taiwan = pd.read_csv(taiwan_data_path, encoding='utf-8-sig')
# 篩選中文評論
df_taiwan = df_taiwan[df_taiwan['語言'] == 'zh'].copy()
print(f"✓ 台灣評論數: {len(df_taiwan):,}")

# 文本前處理
print("\n【Part 1.1】台灣評論文本前處理...")
jieba.load_userdict('../../medical_dict_custom.txt')
with open('../../stopwords_custom.txt', 'r', encoding='utf-8') as f:
...
df_taiwan['cleaned_text'] = df_taiwan['評論內容'].fillna('').apply(clean_text_chinese)
df_taiwan['tokens'] = df_taiwan['cleaned_text'].apply(
    lambda x: [word for word in jieba.cut(x) if word not in stopwords and len(word) > 1]
)
df_taiwan = df_taiwan[df_taiwan['tokens'].str.len() > 0].copy()
print(f"✓ 有效評論: {len(df_taiwan):,}")

# 建立語料庫
texts_taiwan = df_taiwan['tokens'].tolist()
dictionary_taiwan = corpora.Dictionary(texts_taiwan)
dictionary_taiwan.filter_extremes(no_below=5, no_above=0.5, keep_n=None)
corpus_taiwan = [dictionary_taiwan.doc2bow(text) for text in texts_taiwan]
print(f"✓ 台灣語料庫: {len(dictionary_taiwan)} 詞彙, {len(corpus_taiwan)} 文檔")

# 載入台灣 K=7 模型
print("\n【Part 1.2】載入台灣 K=7 LDA 模型...")
taiwan_model_path = '../../results/taiwan_lda_k7/lda_k7_lda_model.pkl'
with open(taiwan_model_path, 'rb') as f:
    lda_taiwan = pickle.load(f)
print(f"✓ 已載入台灣 K=7 模型: {taiwan_model_path}")

# 為每篇評論分配主題
print("\n【Part 1.3】為台灣評論分配主題...")
taiwan_topics = []
for doc_bow in corpus_taiwan:
    topic_dist = lda_taiwan.get_document_topics(doc_bow, minimum_probability=0)
    topic_probs = [prob for _, prob in topic_dist]
    dominant_topic = topic_probs.index(max(topic_probs)) + 1  # 1-indexed
    taiwan_topics.append({
        'dominant_topic': dominant_topic,
        'topic_probability': max(topic_probs)
    })

df_taiwan_result = pd.concat([
    df_taiwan.reset_index(drop=True),
    pd.DataFrame(taiwan_topics)
], axis=1)

# 儲存台灣主題分配結果
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
taiwan_output = f'../../results/taiwan_lda_k7/taiwan_k7_topic_analysis_{timestamp}.csv'
df_taiwan_result.to_csv(taiwan_output, index=False, encoding='utf-8-sig')
print(f"✓ 台灣主題分配已儲存: {taiwan_output}")
...
for dimension, mapping in semantic_mapping.items():
    ...
    # 提取評分
    taiwan_ratings = df_taiwan_result[df_taiwan_result['dominant_topic'] == tw_topic]['評分']
    ...
    effect_size = z_score / np.sqrt(tw_n + us_n)
    ...
    effect_interp = "small" if abs(effect_size) < 0.3 else "medium" if abs(effect_size) < 0.5 else "large"
    print(f"   Effect size r = {abs(effect_size):.3f} ({effect_interp})")
...
# H4: 美國更關注急診等待時間（效率重視）
tw_emergency_count = len(df_taiwan_result[df_taiwan_result['dominant_topic'] == 7])
tw_other_count = len(df_taiwan_result) - tw_emergency_count
...
# 計算比例
tw_emergency_pct = (tw_emergency_count / len(df_taiwan_result)) * 100
...
print(f"\n急診主題比例:")
print(f"  台灣: {tw_emergency_count}/{len(df_taiwan_result)} = {tw_emergency_pct:.1f}%")
...
if p_chi < 0.05:
    print(f"\n✅ H4 支持: 美國評論顯著更關注急診等待時間 (+{us_emergency_pct - tw_emergency_pct:.1f}%, p < {p_chi:.3f})")
else:
...
report_lines.append(f"**台灣樣本數**: {len(df_taiwan_result):,} 筆評論 (K=7)")
...
report_lines.append("|---------|----------------|-------------|-------|-------------|")
report_lines.append(f"| **Taiwan** | {tw_emergency_count:,} | {tw_other_count:,} | {len(df_taiwan_result):,} | {tw_emergency_pct:.1f}% |")
...
report_lines.append(f"**Effect Size**: Cramér's V = {cramers_v:.3f}")
report_lines.append(f"**差異**: 美國比台灣高 {us_emergency_pct - tw_emergency_pct:+.1f} 個百分點")
report_lines.append("")

if p_chi < 0.05:
    report_lines.append(f"✅ **H4 結論**: **支持假設** - 美國評論顯著更關注急診等待時間與效率 (p < 0.001)")
else:
...
report_lines.append(f"- **台灣**: {tw_emerg['TW_Mean']:.2f}★ (所有主題中**最低評分**)")
...
report_lines.append(f"- **台灣**: {tw_nurs['TW_Mean']:.2f}★ (所有主題中**最高評分**)")
report_lines.append(f"- **美國**: {tw_nurs['US_Mean']:.2f}★ (所有主題中**最低評分**)")
...
report_lines.append("- ✅ 優勢: 專業品質評分最高 (4.67★)")
report_lines.append("- ⚠️ 劣勢: 急診評分最低 (1.79★)，比美國低 1.46★")
report_lines.append("- ⚠️ 劣勢: 行政流程效率低 (1.83★)")
report_lines.append("")
report_lines.append("**多支付者制度（美國）**:")
report_lines.append("- ✅ 優勢: 急診評分較高 (3.25★)，雖仍是最大痛點")
report_lines.append("- ⚠️ 劣勢: 護理品質評分最低 (3.00★)")
...
print("="*80)
print(f"\n📊 產出檔案:")
print(f"  1. 台灣主題分配: {taiwan_output}")
print(f"  2. 統計結果 CSV: {stats_output}")
print(f"  3. 統計報告 MD: {report_output}")
print("\n💡 下一步: 請使用這些結果撰寫 Chapter 4")
