...
# -*- coding: utf-8 -*-
"""
美國醫院評論 - 比較5、6、7個主題的LDA模型（簡化版，避免multiprocessing問題）
...
print("="*80)
print("USA Hospital Reviews - LDA Model Comparison (K=5, 6, 7) - Simple Version")
print("美國醫院評論 - LDA模型比較（K=5, 6, 7）- 簡化版")
print("="*80)
...
for num_topics in [5, 6, 7]:
    ...
    # 計算coherence（使用processes=1避免multiprocessing問題）
...
# 儲存比較結果到CSV
comparison_df.to_csv('usa_k5k6k7_comparison.csv', index=False, encoding='utf-8-sig')
print(f"\n✓ Comparison results saved: usa_k5k6k7_comparison.csv")
...
print("="*80)
print(f"\n🏆 Best Model Based on Coherence: K={best_model['num_topics']}")
print(f"🏆 基於一致性分數的最佳模型: K={best_model['num_topics']}")
print(f"   Coherence Score: {best_model['coherence']:.4f}")
print(f"   Perplexity: {best_model['perplexity']:.4f}")

print("\n📊 All Models Comparison | 所有模型比較:")
for r in results:
    print(f"   K={r['num_topics']}: Coherence={r['coherence']:.4f}, Perplexity={r['perplexity']:.4f}")

print("\n💡 Recommendation for Cross-Cultural Analysis | 跨文化分析建議:")
print("   For comparison with Taiwan (K=7), we recommend using K=7")
...
for i, r in enumerate(results):
    axes[0].text(i, r['coherence'] + 0.005, f"{r['coherence']:.4f}",
...
print("   - usa_k5k6k7_comparison.csv")
